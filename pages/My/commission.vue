<template>
	...
	<!-- 		<view class="tui-header-right" :style="{marginTop:top+'px'}" @click="handleOpenDetail">
				查看服务条款
			</view> -->
		</view>
				...
				<view class="detail-item" v-for="(item,index) in pointsList" :key="index"  v-if="!isEmpty">
					...
					<view>
						<view class="detail-content">
							<view>邀请好友获得</view>
							<view>2020-04-22 18:33:12</view>
						</view>
						<view class="add" v-if="tabCurrentIndex == 0">+2</view>
						<view class="reduce" v-else>-2</view>
					</view>
				</view>
				<!-- 加载 -->
	...
	export default {
		...
		methods:{
			...
			async getFlowList(){
				...
				this.page = ++this.page;
				let res = await this.$fetch(this.$api.getFlowLists,{pageNum:this.page,pageSize:10,sign:this.sign,type:4},'GET')
				console.log(res);
				if(res.total){
					this.isEmpty = false
				}else{
